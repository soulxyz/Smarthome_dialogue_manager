...
class IntentRecognizer:
    ...
    def recognize(self, user_input: str, context: Dict, history: List) -> Dict:
        ...
        try:
            ...
            possible_intents = [intent for intent, score in sorted(intent_scores.items(), key=lambda x: x[1], reverse=True)[:3]]
    ...
    def _enhance_with_context(self, user_input: str, intent_scores: Dict[str, float], context: Dict, history: List) -> Dict[str, float]:
        """使用上下文增强意图识别"""
        # 如果上一轮是澄清，提高相关意图的分数
